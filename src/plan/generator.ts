...
function pickSlots(
  ...
  owned: Set<EquipTag>,
  seed = 0,
  sport: SportProfile = NO_SPORT_PROFILE,
): Record<1 | 2 | 3, Record<string, string>> {
  ...
  for (const [slot, base] of Object.entries(POOLS)) {
    ...
    // The sport promotes the lift that most feeds it. A bias, not a
    // rewrite: no matching weights and the pool comes back untouched.
    const legal = bySportTransfer(ordered.filter((id) => canDo(id, owned)), sport)
...
/**
 * What the plan is called, in the words the person used to ask for it.
 *
 * These were "Cut Engine", "Engine Builder", "Hybrid Athlete" — names
 * that sound like something and mean nothing, and that somebody who has
 * never been in a gym would have to decode. The plan is the answer to
 * the goal they picked, so it is named after the goal they picked.
 */
...
export function generatePlan(a: OnboardingAnswers): { plan: PlanConfig; proteinTargetG: number; strategy: string[] } {
  ...
  // Four sport answers are collected. This reads two of them. sport-level
  // and in-season are still unread, and R11 s6 is where they land.
  const ans = a.goalAnswers ?? {}
  const sportProfile = profileForSport(sportOf(ans), positionOf(ans))
  const slots = pickSlots(a.goal, owned, seed, sportProfile)
  ...
  const plan: PlanConfig = {
    ...
    // place 'ball' was ever set was the owner's own preset, so a user
    // who answered Basketball got the generic mode and none of it.
    sportMode: sportProfile.sportMode,
