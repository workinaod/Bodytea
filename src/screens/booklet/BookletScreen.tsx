    ...
    <div className="fixed inset-0 z-[60] flex flex-col bg-bg">
      <div className="mx-auto flex w-full max-w-lg flex-1 flex-col overflow-hidden px-4 pt-[max(env(safe-area-inset-top),16px)]">
        <div className="flex items-center justify-between pb-3">
          ...
          <button onClick={save} className="press-down rounded-full border-2 border-accent-deep bg-accent px-4 py-1.5 text-[12px] font-black text-white [--lip:var(--lip-accent)]">
            Save
          </button>
        </div>
        {problems.length > 0 && (
