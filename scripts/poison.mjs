...
const MUTATIONS = [
  ...
  {
    id: 'rung-ids-move-under-the-climber',
    bug: 'a rung id shifts with the anchor, so a reached rung orphans and un-achieves',
    file: 'src/plan/milestones.ts',
  ...
  {
    id: 'scale-rung-lights-on-one-dry-morning',
    bug: 'a bodyweight rung is reached by a single lucky reading, and stays lit',
    file: 'src/engine/journeyMetrics.ts',
  ...
  {
    id: 'lift-rung-scored-on-an-estimated-max',
    bug: 'a lucky set of twelve at 135 lights the 185 rung',
    file: 'src/engine/journeyMetrics.ts',
  ...
  },
  {
    ...
    bug: 'a rung is never on the rail when it is cleared, so nothing is ever ticked',
    file: 'src/engine/journey.ts',
    find: '      for (const spec of strengthRungs(lift.exerciseId, lift.label, from, cur + 150).slice(0, 7)) {',
    to: '      for (const spec of strengthRungs(lift.exerciseId, lift.label, cur, cur + 150).slice(0, 7)) {',
    spec: 'src/engine/journey.test.ts',
  },
  {
    id: 'rail-points-at-the-furthest-rung',
    bug: 'on a cut the rung four months out is labelled next, and the near one is buried',
    file: 'src/engine/journey.ts',
  ...
  },
  {
    ...
    bug: 'the day a rung was reached is rewritten to today, every time',
    file: 'src/engine/journey.ts',
    find: "    .rail.filter((r) => r.state === 'done' && !hits[r.id])",
    to: "    .rail.filter((r) => r.state === 'done')",
