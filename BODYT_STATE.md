...
| J8 | Learning loop completion: volume autoregulation, schedule fit, exercise fit, intervention follow-up; all suggest-only on the calibration.ts pattern | engines | **in-progress 2026-08-20** (INTERVENTION FOLLOW-UP DONE: every R3 s9.2 row that could be judged against something already shipping. engine/decisions.ts is the ledger, engine/outcomes.ts the verdicts, engine/proposals.ts the neutral vocabulary pinned as a wire format. Judged: the calorie step, the two training adaptations, the deload, the drop-load escalation, the joint-pain substitution, load back on a stated limitation, and the readiness flags themselves. VOLUME AUTOREGULATION DONE: engine/ceiling.ts, both directions, delta in the ledger and the base left as the researched constant. STILL OPEN: schedule fit and exercise fit, which are R3 s9.2's schedule-change and exercise-variation rows and need the intervention built before there is an outcome to judge. J7's trainsOnWeekday and daysSinceRegion are the inputs waiting for them. BLOCKED ON OWNER: the equipment-substitute default, section 5) | J7 | Algorithm session |
...
### 2026-08-20 · J8 · volume autoregulation, both directions
  The board defines J8 as volume autoregulation, schedule fit, exercise fit and intervention
  follow-up. The follow-up half closed with slice 8, so this is the first of the three that
  needed the INTERVENTION built rather than an outcome pointed at something that already
  shipped. J7 left workCapacity waiting for exactly this.
  R3 s5 is unusually cautious because s5.3 says outright that volume autoregulation is the
  under-evidenced half of the autoregulation literature. So: one fractional set and never two,
  drift capped at two either way, ONE change at a time, and the first move is down. Lowering
  needs one of four signals; raising needs all six of s5.3's conditions plus a goal check from
  s5.6, because volume is size's first lever and strength's last one, fat loss should not chase
  it at all, and the explosive goals live on freshness.
  THE DELTA LIVES IN THE LEDGER AND THE BASE STAYS THE RESEARCHED CONSTANT. R3 s5.5 guard 2,
  and this repo already carries the scar it is written from: reps.ts once walked 25 lb to zero
  in five weeks by letting each reduction become the next baseline. A lowered ceiling that
  quietly becomes the new base does that to a muscle instead of a lift. The focus bonus stays
  out of the drift so the muscle the day was built around does not move twice as fast.
  THREE THINGS THE GUARDS CAUGHT, NOT ME.
  (1) The fortnight guard has to be GLOBAL, not per muscle: a squat set serves quads and glutes
  both, so taking one off for the quads has already cut the glutes' exposure and offering the
  glutes next is the same fix counted twice. The tests found it by returning glutes when I
  expected silence.
  (2) attendance() seeded its worst gap from the OLDEST session rather than the newest, so it
  measured the whole four-week window as one gap and the raise could not fire for anybody. The
  only symptom would have been a rule that never seemed to apply, which is the quietest kind of
  dead code. Caught by the ideal-case test, and it has its own mutation.
  (3) I wrote the fortnight guard out twice, once per direction, and the mutation harness
  refused to run because the anchor now matched in two places. Fifth two-definitions bug this
  session and the second one a guard found rather than a review. One tooSoon() now.
  ALSO: raise-off-absent-evidence survived its first run. The no-effort test deletes every rir,
  so the region never enters the tally and the length guard is never reached; the case it
  actually protects is ONE honest answer being read as permission. Its own test.
  A PROCESS SCAR WORTH RECORDING: I edited the tree while poison was running in the background.
  It mutates and restores from git, so it silently reverted a whole wiring change and left the
  next e2e run reporting BUILD FAILED against code that was fine. Do not touch the working tree
  while the harness is running, and check `git status` before trusting a green run.
  Validation: tsc -b clean, **1,799/1,799 unit** (19 new), build green, sim 20 personas,
  **98/98 e2e**, **poison 264/264 unit + 18/18 e2e**.
  NEXT: J8's remaining two, schedule fit and exercise fit, which are R3 s9.2's schedule-change
